        ...
        <div className="min-h-screen">
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
                ...
                <button onClick={() => navigate(-1)} className="mb-6 flex items-center gap-1.5 text-sm text-app-text-light hover:text-app-green transition-colors">
                    <ArrowLeftIcon className="size-4"></ArrowLeftIcon>back
                </button>
                <div className="bg-white/50 rounded-2xl overflow-hidden">
                    <div className="grid md:grid-cols-2 gap-0">
                        <div className="relative flex-center p-8 md:p-12 min-h-[320px] md:min-h-[480px]">
                            <img src={product.image} alt={product.name} className="max-h-[360px] w-auto object-contain"></img>
                            <div className="absolute top-5 left-5 flex flex-wrap gap-1.5">
                                {product.isOrganic && (<span className="flex items-center gap-1 px-2.5 py-1 text-xs font-semibold rounded-full bg-app-green text-white">
                                    <LeafIcon className="w-3 h-3"></LeafIcon>organic</span>)}
                                {product.discount > 0 && (
                        ...
                        <div className="p-6 md:p-10 flex flex-col justify-center">
                                ...
                                <div className="flex items-center gap-2 mb-5">
                                    <div className="flex items-center gap-0.5">
                                        {[1, 2, , 3, 4, 5].map((star: any) => (
                                            <StarIcon key={star} className={`w-4 h-4 ${star <= Math.round(product.rating) ? 'text-app-warning fill-app-warning' : 'text-app-border'}`}></StarIcon>
                                        ))}
                                    </div>
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <div className="flex items-center border border-app-border rounded-xl overflow-hidden"></div>
                                <button onClick={handleMinus} className="p-2 hover:bg-app-cream transition-colors">
                                ...
                                }} className={`flex-1 py-3 font-semibold rounded-xl transition-colors flex-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed active:scale-[0.98] ${inCart ? 'bg-cream-app text-app-green border border-app-green' : 'bg-app-orange text-white hover:bg-app-orange-dark'}`}>
                ...
                {product.reviewCount > 0 && <DummyReviewsSection product={product}></DummyReviewsSection>}
                {relatedProducts.length > 0 && (
                    <section className="mt-12 mb-44">
                        <div className="flex items-center justify-between mb-6">
                            ...
                            <Link className="text-sm font-semibold text-app-orange hover:text-app-orange-dark flex items-center gap-1 transition-colors" to={`/products?category=${product.category}`}>view all <ArrowRightIcon className='size-4'></ArrowRightIcon></Link>
                        </div>
                        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4 xl:gap-8">
                            {relatedProducts.slice(0, 5).map((rp) => (
                                <ProductCard key={rp._id} product={rp} ></ProductCard>
                            ))}
                        </div>
                    </section>
                )}
