        ...
        return (
            <div className="min-h-screen bg-app-cream flex-center">
                <div className="text-center">
                    ...
                    <p className="text-xl font-semibold text-app-green mb-2">add some products to checkout</p>
        ...
        <div className="min-h-screen bg-app-cream">
            <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                <button onClick={() => navigate(-1)} className="flex items-center gap-2 text-sm text-app-text-light hover:text-app-green mb-6 transition-colors">
                    <ArrowLeft className="size-4"></ArrowLeft>back
                </button>
                    ...
                    {steps.map((s, i) => (
                        <div key={s.key} className="flex items-center gap-2">
                            <button onClick={() => setStep(s.key)} className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-medium transition-colors ${step === s.key ? 'bg-app-green text-white' : 'bg-white text-app-text-light'}`}>
                                <s.icon className="size-4"></s.icon> {s.label}
                                {i < steps.length - 1 && <ChevronRightIcon className="size-4 text-app-text-light"></ChevronRightIcon>}
                            </button>
                        </div>
                    ))}
                </div>
                <div className="grid md:grid-cols-3 gap-6">
                    <div className="md:col-span-2">
                        {step === 'address' && <CheckoutAddress address={address} setAddress={setAddress}
                            setStep={setStep} user={user} ></CheckoutAddress>}
                        {step === 'payment' && <CheckoutPayment paymentMethod={paymentMethod} setPaymentMethod={setPaymentMethod} setStep={setStep}  ></CheckoutPayment>}
                        {step === 'review' && <CheckoutReview address={address} items={items}
                            handlePlaceOrder={handlePlaceOrder} loading={loading} total={total} ></CheckoutReview>}
                    </div>
