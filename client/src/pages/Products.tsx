...
const Products = () => {
    ...
    return (
        <div className="min-h-screen bg-app-cream">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
                ...
                <div className="flex gap-8 xl:gap-10">
                    <aside className="hidden lg:block w-64 shrink-0">
                        <div className="bg-white rounded-2xl p-4 sticky top-24">
                            <FilterPanel categories={categoriesData} category={category} organic={organic}
                                minPrice={minPrice} maxPrice={maxPrice} updateFilter={updateFilter}
                                clearFilters={clearFilters} hasFilters={hasFilters}></FilterPanel>
                        </div>
                    </aside>
                    <main className="flex-1">
                        <div className="flex items-center justify-between mb-6">
                            ...
                            <div className="flex flex-col lg:items-center gap-3">
                                <button onClick={() => setMobileFiltersOpen(true)} className="lg:hidden flex items-center gap-2 px-3 py-2 text-sm bg-white rounded-xl border border-app-border-hover:bg-app-cream transition-colors">
                                    <SlidersHorizontal className="size-4"></SlidersHorizontal>filters
                                </button>
                                <div className="relative">
                                    ...
                                    <ChevronDown className="absolute right-2.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-app-text-light pointer-events-none"></ChevronDown>
                                </div>
                            </div>
                        </div>
                        {
                            loading ? (<Loading></Loading>) : products.length === 0 ?
                        ...
                        {totalPages > 1 && (
                            <div className="flex-center gap-2 mt-16">{Array.from({ length: totalPages }).map((_, i) => (
                                <button key={i} onClick={() => { updateFilter('page', String(i + 1)); scrollTo(0, 0) }} className={`size-9 rounded-lg text-sm font-medium tranistion-colors ${page === i + 1 ? 'bg-app-green text-white' : 'bg-white text-app-text-light hover:bg-app-cream'}`}>{i + 1}</button>
                            ))}</div>
                        )}
                    </main>
                </div>
            ...
            {mobileFiltersOpen && (
                <>
                    <div className="fixed inset-0 bg-black/40 z-50" onClick={() => setMobileFiltersOpen(false)}></div>
