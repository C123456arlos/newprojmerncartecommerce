                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                    <div>
                        <Link to={'/'} className="flex items-center gap-2 mb-4">
                            <BikeIcon className="size-6 text-white"></BikeIcon>
                        ...
                        <div className="flex gap-3">{footerData.brand.socials.map((social, i) => (
                            <a href={social.link} key={i} className="size-9 rounded-lg bg-white/10 flex-center hover:bg-white/2"><social.icon className="size-4"></social.icon></a>
                        ))}
                        </div>
                    </div>
                        ...
                        <div key={i}>
                            ...
                            <ul className="space-y-2.5">
                                {section.links.map((link, i) => (
                                    <li key={i}>
                                        {link.to ? (<Link to={link.to} className="text-sm text-white/70 hover:text-white">{link.label}</Link>) : (<a href={link.href} className="text-sm text-white/70 hover:text-white">{link.label}</a>)}
                                    </li>
                                ))}
                            </ul>
                        </div>
                            ...
                            {footerData.contact.map((item, i) => {
                                ...
                                return (
                                    <li key={i} className="flex gap-3 text-sm text-white/70">
                                        <Icon className="size-4 text-white"></Icon> {item.text}
                                    </li>
                                )
                            })}
                        </ul>
